...
%epsilonV：模拟中长度为K-1的耦合强度矢量，我们使用相同的耦合强度epsilon = 0,1,2,3,4,5,6
% OUTPUTS
% - xM : matrix (n x K) of the X-variables from each Lorenz subsystem  来自每个Lorenz子系统的X变量的矩阵（n×K）
...
ntrans = 1000;
T = (n+ntrans)*ts;
[t,yM] = ode45(@odefun,(0:ts:T),x0V,[],omega); %求解非刚性微分方程 - 中阶方法    龙格库塔法
